...
void ArmMaker::makeFullArm(double length, const Eigen::Vector3d location)
{
	...
	int index_thumb = rightArm->getIndexOf(rightArm->getBodyNode("thumb metacarpal")->getParentJoint()->getDof(0));
...
void ArmMaker::makeArmPart(double length)
{
	...
	bn = skel.univCylinder(currentArm, bn, "arm lower part right",Eigen::Vector3d::UnitX(), Eigen::Vector3d::UnitZ(), arm_rad/2, arm_len, Eigen::Vector3d(0, 0, -arm_len/2), Eigen::Vector3d(0,0,arm_len/2), mass, color);
...
void ArmMaker::makePalm()
{
	...
	bn = skel.univBox(currentArm, currentParent, "palm", Eigen::Vector3d::UnitX(),Eigen::Vector3d::UnitY(), boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), Eigen::Vector3d(0,0,0.15), mass, color);
...
void ArmMaker::makeSingleFinger(int idx)
{
	...
	bn = skel.univBox(currentArm, currentParent, "metacarpal"+std::to_string(idx), Eigen::Vector3d::UnitX(), Eigen::Vector3d::UnitY(), boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), Eigen::Vector3d(offset, 0, palmSize[2]/2), mass_long , color);
	boxsize[2] = 0.05;
	bn = skel.revolBox(currentArm, bn, "proxphalanx"+std::to_string(idx), Eigen::Vector3d::UnitX(), boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), Eigen::Vector3d(0,0,fing_len/2), mass_short, color);
	fing_len = 0.05;
	boxsize[2] = 0.03;
	bn = skel.revolBox(currentArm, bn, "distphalanx"+std::to_string(idx), Eigen::Vector3d::UnitX(), boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), Eigen::Vector3d(0,0,fing_len/2), mass_short, color);		
...
void ArmMaker::makeThumb()
{
	...
	bn = skel.univBox(currentArm, currentParent, "thumb metacarpal", Eigen::Vector3d::UnitX(), Eigen::Vector3d::UnitY(), boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), offset,mass_long ,color);
	// bn = skel.ballBox(currentArm, currentParent, "thumbball", boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), offset, mass_long, color);
	boxsize[2] = 0.05;
	bn = skel.univBox(currentArm, bn, "thumb proxphalanx", Eigen::Vector3d::UnitX(), Eigen::Vector3d::UnitY(), boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), Eigen::Vector3d(0,0,fing_len/2),mass_short ,color);	
	...
	fing_len = 0.05;
	boxsize[2] = 0.03;
	bn = skel.revolBox(currentArm, bn, "thumb distphalanx", Eigen::Vector3d::UnitY(), boxsize, Eigen::Vector3d(0,0,-boxsize[2]/2), Eigen::Vector3d(0,0,fing_len/2), mass_short, color);		
	if(currentDirection == LEFT)
		bn = skel.weldBox(currentArm, bn, "thumbpatch", patch, Eigen::Vector3d(0,0,0), Eigen::Vector3d(boxsize[0]/2,0, boxsize[2]/2-patch[2]/2), epsilon, dart::Color::Orange());
	else
		bn = skel.weldBox(currentArm, bn, "thumbpatch", patch, Eigen::Vector3d(0,0,0), Eigen::Vector3d(-boxsize[0]/2,0, boxsize[2]/2-patch[2]/2), epsilon, dart::Color::Orange());
